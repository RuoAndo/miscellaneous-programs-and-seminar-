...
./listup.pl | grep gz | grep tar | grep bind > postgres_bind_list

date=`date '+%y_%m_%d_%H_%M_%S'`
touch processing_time_postgres_bind_$date

TESTFILE=postgres_bind_list
while read line; do
    cp $line .
    ...
    #echo $currentdir4 "," $d >> dblist_ryaku_$date
    ...
    echo $currentdir4 ":" $mongo_counter ":" $proctime >> processing_time_postgres_bind_$date
