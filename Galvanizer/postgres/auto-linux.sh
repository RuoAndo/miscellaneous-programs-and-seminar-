...
./listup.pl | grep gz | grep linux | grep tar > postgres_linux_list

date=`date '+%y_%m_%d_%H_%M_%S'`
touch processing_time_postgres_linux_$date

TESTFILE=postgres_linux_list
while read line; do
    cp $line .
    ...
    echo $currentdir4 ":" $mongo_counter ":" $proctime >> processing_time_postgres_linux_$date
