...
./listup.pl | grep gz | grep linux > list-mongodb-linux
...
touch processing_time_linux_mongodb_$date

TESTFILE=list-mongodb-linux
while read line; do
    cp $line .
    ...
    echo $currentdir4 ":" $mongo_counter ":" $proctime >> processing_time_linux_mongodb_$date
