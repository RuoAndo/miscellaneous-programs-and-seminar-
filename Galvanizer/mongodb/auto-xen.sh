...
./listup.pl | grep gz | grep xen | grep -v mongdb > list-mongodb-xen

rm -rf processing_time

date=`date '+%y_%m_%d_%H_%M_%S'`
touch processing_time_xen_mongodb_$date

TESTFILE=list-mongodb-xen
while read line; do
    cp $line .
    ...
    echo $currentdir4 ":" $mongo_counter ":" $proctime >> processing_time_xen_mongodb_$date
