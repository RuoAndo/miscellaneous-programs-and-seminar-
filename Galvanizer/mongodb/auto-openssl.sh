...
./listup.pl | grep gz | grep openssl > list-mongodb-openssl
#./listup.pl | grep gz | grep bind | grep 9.9.7 >> gzlist
#./listup.pl | grep gz | grep bind | grep 9.9.9 >> gzlist

#rm -rf processing_time

date=`date '+%y_%m_%d_%H_%M_%S'`
touch processing_time_openssl_mongodb_$date

TESTFILE=list-mongodb-openssl
while read line; do
    cp $line .
    ...
    echo $currentdir4 ":" $mongo_counter ":" $proctime >> processing_time_openssl_mongodb_$date
