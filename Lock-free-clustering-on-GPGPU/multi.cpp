...
typedef tbb::concurrent_hash_map<long, int> iTbb_Vec_timestamp_1;
static iTbb_Vec_timestamp_1 TbbVec_timestamp_1;

typedef tbb::concurrent_hash_map<long, int> iTbb_Vec_timestamp_2;
...
int traverse_file(char* filename, int thread_id) {
	...
	key[row] = stoull(tms);
	value[row] = stol(rec[1]);
	value_2[row] = stol(rec[2]);
    }
    ...
    for(int i = 0; i < new_size; i++)
      {
	iTbb_Vec_timestamp_1::accessor tms;
	TbbVec_timestamp_1.insert(tms, key_out[i]);
    ...
    transfer(key, value_2, key_out, value_out, kBytes, vBytes, data.size(), &new_size, thread_id);
   
    for(int i = 0; i < new_size; i++)
      {
	iTbb_Vec_timestamp_2::accessor tms;
	TbbVec_timestamp_2.insert(tms, key_out[i]);
	tms->second += value_out[i];
      }
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    for(auto itr = TbbVec_timestamp_1.begin(); itr != TbbVec_timestamp_1.end(); ++itr) {
      final.insert(std::make_pair((unsigned long long)(itr->first), long(itr->second)));
    }
    
    ofstream outputfile("tmp-1");
    ...
    std::map<unsigned long long, long> final_2;
    
    for(auto itr = TbbVec_timestamp_2.begin(); itr != TbbVec_timestamp_2.end(); ++itr) {
      final_2.insert(std::make_pair((unsigned long long)(itr->first), long(itr->second)));
    }
    
    ofstream outputfile2("tmp-2");
    for(auto itr = final_2.begin(); itr != final_2.end(); ++itr) {

      std::string timestamp = to_string(itr->first);

      outputfile2 << timestamp.substr(0,4) << "-" << timestamp.substr(4,2) << "-" << timestamp.substr(6,2) << " "
		 << timestamp.substr(8,2) << ":" << timestamp.substr(10,2) << ":" << timestamp.substr(12,2)
		 << "." << timestamp.substr(14,3) << ","
		 << itr->second << endl; 
    }
